...
$app->post('/api/search', function($request, $response){
        $input = $request->getParsedBody();
        $stmt = $this->db->prepare("INSERT INTO Saves (originCity, destinationCity, date, userId, cityId) VALUES(:originCity, :destinationCity, NOW(), :userId, :cityId)");
        $stmt->bindParam("userId", $input['userId']);
        $stmt->bindParam("originCity", $input['originCity']);
        $stmt->bindParam("destinationCity", $input['destinationCity']);
        $stmt->bindParam("cityId", $input['cityId']);
        $stmt->execute();
...
$app->post('/api/bubble', function($request, $response, $args){
        $input = $request->getParsedBody();
        $query = "";
        foreach($input['options'] as $selected)
        {
        ...
        $origin_city_costs = $this->db->prepare("SELECT " .$query . " as total_cost FROM Cities WHERE name=:origin");
        $origin_city_costs->bindParam("origin", $origin);
        $origin_city_costs->execute();
        $cost_results_orig = $origin_city_costs->fetchAll();
        //echo $origin . ": " . json_encode($cost_results_orig);
        $reponse =  $origin . ": " . json_encode($cost_results_orig);
        foreach($input['destinationCity'] as $destination)
        {
                $dest_city_costs = $this->db->prepare("SELECT " . $query . " as " . $destination . " FROM Cities WHERE name=:destination");
                $dest_city_costs->bindParam("destination", $destination);
                $dest_city_costs->execute();
                $cost_results_dest =  $dest_city_costs->fetchAll();
                //echo $destination . ": " . json_encode($cost_results_dest);
